...
func (s *ImageService) PullImage(pullParameter ServicePullModel) error {
	...
	repoOut := filepath.Join(env.LayerRootDir, strings.ReplaceAll(imageRef.repository, "/", "_"), imageRef.reference)
	...
	if err := s.copyFile(
		...
		filepath.Join(repoOut, "config.json"),
	); err != nil {
		return err
	}
	...
	if err := s.applyLayers(filepath.Join(repoOut, "rootfs"), layerPaths); err != nil {
		return err
	}
...
func (s *ImageService) createOutputDirectory(repoOut string) error {
	// image root
...
func (s *ImageService) parseImageRef(imageStr string) (imageRefParts, error) {
...
func (s *ImageService) getBearerChallenge(ctx context.Context, client *http.Client, registry string) (realm, service string, err error) {
	u := "https://" + registry + "/v2/"
...
func (s *ImageService) splitCommaPreserveQuotes(str string) []string {
	var out []string
...
func (s *ImageService) fetchToken(ctx context.Context, client *http.Client, realm, service, scope string) (string, error) {
...
func (s *ImageService) fetchManifest(ctx context.Context, client *http.Client, ref imageRefParts, token string) (body []byte, mediaType string, err error) {
...
func (s *ImageService) storeManifest(repoOut string, data []byte, filename string) error {
...
func (s *ImageService) isManifestListMediaType(ct string) bool {
	ct = strings.ToLower(strings.TrimSpace(strings.Split(ct, ";")[0]))
	return ct == "application/vnd.docker.distribution.manifest.list.v2+json" ||
		ct == "application/vnd.oci.image.index.v1+json"
}

func (s *ImageService) pickFromManifestList(b []byte, targetOs, targetArch string) (string, error) {
	var ml manifestList
...
func (s *ImageService) parseSingleManifest(b []byte) (*singleManifest, error) {
...
func (s *ImageService) digestToFilename(d string) string {
	// sha256:abcd... -> sha256_abcd...
	return strings.ReplaceAll(d, ":", "_")
}

func (s *ImageService) downloadBlobVerified(ctx context.Context, client *http.Client, ref imageRefParts, token, digest, dest string) error {
...
func (s *ImageService) applyLayers(rootfs string, layerBlobPaths []string) error {
...
func (s *ImageService) applyOneLayer(rootfs, layerBlobPath string) error {
...
func (s *ImageService) writeFileFromTar(dstPath string, r io.Reader, mode os.FileMode) error {
	tmp := dstPath + ".tmp"
...
func (s *ImageService) joinRoot(rootfs, rel string) (string, error) {
...
func (s *ImageService) removeAllChildren(dir string) error {
	st, err := os.Stat(dir)
	if err != nil {
